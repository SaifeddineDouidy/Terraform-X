...
\title{\textbf{HolisticX Cloud Architecture}}
...
The architecture is a modern microservices-based system built for scalability, resilience, and security on AWS. It follows a layered design consisting of a secure network layer, a container-based application layer, a managed data layer, and a robust edge layer for handling incoming requests. A key update is the centralization of the Keycloak identity provider on a dedicated, highly available Fargate cluster.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{overall_architecture.png} % Placeholder for a new diagram
...
\begin{itemize}
    ...
    \item \textbf{Secrets Management:} Database access credentials and other sensitive information are securely stored and retrieved using AWS Secrets Manager. Terraform automates the creation and initial population of these secrets.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{user_request_flow.png} % Placeholder for a new diagram
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{keycloak_login_flow.png} % Placeholder for a new diagram
